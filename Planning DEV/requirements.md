...
SayknowMind는 오픈소스(MIT) 개인 에이전틱 세컨드 브레인 플랫폼이다. "Everything you say, we know, and mind forever." 슬로건 아래, 사용자가 수집한 모든 지식을 로컬 우선(Private Mode)으로 저장·검색·활용할 수 있는 풀스택 크로스 플랫폼 시스템이다. 100% 무료 오픈소스 3-Layer RAG 스택을 핵심으로 하며, 멀티 에이전트 오케스트레이션, MCP Server를 통한 외부 AI 플랫폼 연동, 선택적 분산 공유(Shared Mode)를 제공한다. 프로덕션 v0.1.0 출시를 목표로 한다.

### 3-Layer RAG 스택

1. **EdgeQuake**: Rust 기반 검색 엔진 (대시보드·PDF vision·6가지 쿼리 모드) → 빠르고 가벼움
2. **UltraRAG**: 복잡한 Agentic 파이프라인 (자동 카테고리 생성, ZeroClaw wrapping, MCP Skill)을 YAML로 쉽게 정의
3. **zvec**: 초경량 인-프로세스 벡터 엔진 → EdgeQuake의 pgvector와 함께 하이브리드 사용 가능
- **ZeroClaw_Runtime**: Rust 기반 Agent Runtime (v0.1.7). 안전성이 검증된 기능만 사용
- **LangGraph_Orchestrator**: Stateful 멀티 에이전트 오케스트레이션 프레임워크
- **sayknow-ai-server**: AI 추론 엔진 서버 (포트 4000). OpenRouter, Z.AI, Cloudflare cascade fallback, WebLLM, Ollama 지원. Intelligence System (tracing, analytics, feedback) 내장
...
- **EdgeQuake_Engine**: Rust 기반 검색 엔진 (v0.5.1). Apache AGE(그래프 DB) + pgvector(벡터 DB)를 내장하며, 6가지 Query Mode를 제공
- **UltraRAG_Framework**: YAML 기반 Agentic RAG 파이프라인 프레임워크
- **zvec_Engine**: 초경량 인-프로세스 벡터 검색 엔진
- **ZeroClaw_Runtime**: Rust 기반 Agent Runtime (v0.1.7). 안전성이 검증된 기능만 사용
- **LangGraph_Orchestrator**: Stateful 멀티 에이전트 오케스트레이션 프레임워크
- **Ingestion_Pipeline**: crawl4ai, vakra-dev/reader, Scrapling, tf-playwright-stealth를 조합한 데이터 수집 파이프라인
- **Private_Mode**: 모든 데이터를 100% 로컬에 저장하며 외부 네트워크 접근을 차단하는 기본 운영 모드
- **Shared_Mode**: IPFS Kubo + age 암호화 + Tailscale Discovery 기반 100% 무료 오픈소스 분산 공유 모드
...
- **Mobile_App**: Capacitor로 빌드된 Android/iOS 모바일 애플리케이션
- **SDK**: Python, TypeScript, Go 언어로 제공되는 소프트웨어 개발 키트
- **AntiBot_Module**: tiagozip/cap 기반 self-hosted PoW CAPTCHA 모듈
...
8. THE zvec_Engine SHALL EdgeQuake_Engine의 pgvector와 함께 하이브리드 벡터 검색을 제공하여 메모리 효율적인 검색을 지원한다
...
1. THE Shared_Mode SHALL 기본적으로 완전 무료 방식(IPFS Kubo + age 암호화 + Tailscale Discovery)을 사용한다
2. WHEN 사용자가 Document를 공유하면, THE Shared_Mode SHALL age로 암호화한 후 IPFS Kubo에 업로드한다
3. WHEN 공유 링크를 생성하면, THE Shared_Mode SHALL Tailscale 네트워크 또는 공개 IPFS 게이트웨이를 통해 접근 가능하게 한다
4. WHEN 사용자가 공유 권한을 철회하면, THE Shared_Mode SHALL IPFS unpin + 메타데이터 삭제로 즉시 무효화한다
5. WHEN 권한이 없는 사용자가 접근하면, THE Shared_Mode SHALL 접근을 차단하고 권한 부족 메시지를 표시한다
6. THE AntiBot_Module SHALL Shared Mode에서 tiagozip/cap (self-hosted PoW CAPTCHA)을 기본 활성화한다
...
2. THE Mobile_App SHALL Capacitor 프레임워크를 기본으로 사용하여 Android/iOS를 지원한다
...
6. WHEN 네트워크가 복구되면, THE Mobile_App SHALL 오프라인 중 수집된 데이터를 자동 동기화한다
7. THE Mobile_App SHALL 푸시 알림으로 Agent 제안·수집 완료를 알려준다

### 요구사항 14: Agent Runtime 및 UltraRAG 파이프라인
...
9. THE UltraRAG_Framework SHALL YAML 파일로 Agentic 파이프라인(자동 카테고리, ZeroClaw wrapping, MCP Skill 등)을 선언적으로 정의한다
10. THE UltraRAG_Framework SHALL 정의된 YAML을 ZeroClaw_Runtime 위에서 실행한다
11. WHEN 새로운 YAML 파이프라인이 등록되면, THE UltraRAG_Framework SHALL 구문 검증을 수행한다
...
11. THE SayknowMind_System SHALL sayknow-ai-server와의 통신에 RSA 서명 기반 인증을 사용한다
12. THE SayknowMind_System SHALL 자동 백업 기능을 제공하며, 주기적 스냅샷을 생성한다
13. THE AntiBot_Module SHALL tiagozip/cap (self-hosted PoW CAPTCHA)을 Shared Mode에서 기본 활성화한다
2. WHEN 페이지가 로드되면, THE Frontend SHALL 초기 렌더링을 1초 이내에 완료한다
3. THE SayknowMind_System SHALL 10만 개 이상의 Document를 저장하고 검색 성능을 유지한다
4. THE SayknowMind_System SHALL 모든 저장 데이터를 AES-256 암호화로 보호한다
5. THE SayknowMind_System SHALL OWASP Top 10 보안 취약점에 대한 방어 조치를 적용한다
...
9. THE SayknowMind_System SHALL API 응답 형식으로 JSON을 사용한다
10. FOR ALL 유효한 API 응답 객체에 대해, JSON으로 직렬화한 후 역직렬화하면 원본과 동일한 객체를 생성한다 (라운드트립 속성)
11. THE SayknowMind_System SHALL Ollama를 기본 로컬 LLM으로 사용한다
12. THE SayknowMind_System SHALL 자동 백업 기능을 제공하며, 주기적 스냅샷을 생성한다
13. THE AntiBot_Module SHALL tiagozip/cap (self-hosted PoW CAPTCHA)을 Shared Mode에서 기본 활성화한다
...
**사용자 스토리:** 개발자로서, 코드베이스의 일관성과 국제 협업 가능성을 위해 모든 소스 코드와 주석이 영어로 작성되길 원한다.

#### 인수 조건

1. THE SayknowMind_System SHALL 모든 소스 코드(변수명, 함수명, 클래스명 등)를 영어로만 작성한다
2. THE SayknowMind_System SHALL 모든 코드 주석을 영어로만 작성한다
3. THE SayknowMind_System SHALL 커밋 메시지를 영어로 작성한다
4. THIS RULE SHALL UI 텍스트, i18n 리소스 파일, 사용자 문서에는 적용되지 않는다 (다국어 허용)


### 요구사항 18: AI 모델 통합 (sayknow-ai-server 연동)

**사용자 스토리:** 사용자로서, 기존 sayknow-ai-server의 AI 기능을 활용하여 문서 요약, Entity 추출, 카테고리 제안, Agentic Query를 수행하고 싶다.

#### 인수 조건

1. THE SayknowMind_System SHALL sayknow-ai-server (포트 4000)를 AI 추론 엔진으로 사용한다
2. THE SayknowMind_System SHALL sayknow-ai-server의 `/ai/chat` 엔드포인트를 통해 AI 요청을 전송한다
3. THE SayknowMind_System SHALL 문서 요약, Entity 추출, 카테고리 제안 작업에 경량 모델(Qwen2.5-0.5B 또는 Phi-3.5-mini)을 지정한다
4. THE SayknowMind_System SHALL Agentic Query 작업에 고성능 모델(Qwen3.5-3B 또는 Step 3.5 Flash)을 지정한다
5. THE SayknowMind_System SHALL sayknow-ai-server의 cascade fallback 기능을 활용하여 모델 장애 시 자동 전환한다
6. WHEN sayknow-ai-server 연결이 실패하면, THE SayknowMind_System SHALL 사용자에게 오류 메시지를 표시하고 재시도 옵션을 제공한다
7. THE SayknowMind_System SHALL sayknow-ai-server의 Intelligence System (tracing, analytics, feedback)을 활용한다
8. THE SayknowMind_System SHALL 환경 변수로 sayknow-ai-server URL과 API 키를 설정한다
9. THE SayknowMind_System SHALL sayknow-ai-server의 WebLLM, Ollama, OpenRouter 등 모든 지원 모델을 활용할 수 있다
10. WHEN 사용자가 설정에서 AI 모델을 변경하면, THE SayknowMind_System SHALL sayknow-ai-server에 해당 모델을 요청한다
